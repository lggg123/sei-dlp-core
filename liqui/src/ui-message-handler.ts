...
export class UIMessageHandler {
  ...
  private async generateCharacterResponse(memory: Memory, state: State): Promise<string> {
    ...
    if (messageText.includes('sei') || messageText.includes('chain')) {
      return `⚡ SEI's 400ms finality is perfect for active liquidity management! Unlike Ethereum's $50+ rebalancing costs, SEI enables frequent optimization at ~$0.15 per transaction. This lets us capture more fees and reduce impermanent loss.`;
    }
    
    if (messageText.includes('vault') || vaultAddress) {
      return `🎯 I'm analyzing your vault performance on Chain 713715. SEI's parallel execution and fast finality make it ideal for concentrated liquidity strategies. Would you like me to check current utilization and suggest rebalancing?`;
    }
    
    if (messageText.includes('apy') || messageText.includes('yield')) {
      return `💰 Current vault yields are looking strong! Our AI models target 15-25% APY through optimal range management and cross-protocol arbitrage. SEI's speed advantage lets us capture opportunities other chains miss.`;
    }
    
    if (messageText.includes('risk') || messageText.includes('safe')) {
      return `🛡️ Risk management is key in DeFi. I monitor impermanent loss, volatility clustering, and liquidity depth. SEI's fast finality allows rapid position adjustments when market conditions change.`;
    }
